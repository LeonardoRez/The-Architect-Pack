...
function MakeHedgeType(data)
    ...
    local function ondeploywall(inst, pt, deployer)
        local wall = SpawnPrefab("hedge_block_aged") 
        if wall ~= nil then 
    ...
    return Prefab("hedge_block_aged", fn, assets, prefabs),
        Prefab("hedge_block_aged_item", itemfn, assets, {"hedge_block_aged_item_placer", "collapse_small"}),
		MakePlacer("hedge_block_aged_item_placer", "hedge", "hedge1_build", "growth2", false, false, true, nil, nil, "eight")
end

local hedgeprefabs = {}
local hedgedata =
{   
    { name = "hedge_block_aged",	hedgetype = 1, material = "grass", tags = { "grass", "hedge" },	loot = "cutgrass", maxloots = 2, maxhealth = TUNING.STONEWALL_HEALTH, buildsound = "dontstarve/common/place_structure_straw" },
}
